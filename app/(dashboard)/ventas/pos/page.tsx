...
export default function POSPage() {
  ...
  useStore(subscribeProductCategories, getProductCategoriesData);
  ...
  const addToCart = (item: StoreInventoryItem) => {
    ...
    if (!isCashOpen) {
      toast.error('Caja cerrada', { id: 'cash-closed', description: 'Debes abrir la caja antes de vender.' });
      return;
    }
    ...
    setCart((prev) => {
      ...
      if (existing) {
        if (existing.quantity >= item.stockUnits) {
          toast.error('Stock insuficiente', { id: `stock-${item.productId}` });
          return prev;
        }
    ...
    setCart((prev) =>
      prev
        .map((line) => {
          ...
          const stockItem = MOCK_STORE_INVENTORY.find((i) => i.productId === line.productId);
  ...
  const handleConfirmPayment = () => {
    if (paymentMethod === 'efectivo' && cashReceivedNum < cartTotal) {
      toast.error('Monto insuficiente', { id: 'insufficient-cash' });
      return;
    }
    ...
    <div className="space-y-4">
      ...
      {!isCashOpen && (
        <div className="flex items-center gap-3 rounded-lg border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/30 p-3">
          ...
          <button
            onClick={() => router.push('/ventas/pos/caja')}
