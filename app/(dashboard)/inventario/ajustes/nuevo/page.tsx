...
export default function NuevoAjustePage() {
    ...
    if (productId) {
      ...
      if (product) {
        setLines([{
          ...
          currentStock: product.stock.existence,
          adjustmentQty: 1,
  ...
  // Calculate totals
  const totalItems = lines.reduce((sum, l) => sum + Math.abs(l.adjustmentQty), 0);
  const totalValue = lines.reduce((sum, l) => sum + Math.abs(l.adjustmentQty) * l.costCIF, 0);
  ...
  const validateForm = () => {
    ...
    if (adjustmentType === 'negativo') {
      for (const line of lines) {
        const validation = validateStockOperation(line.productId, line.adjustmentQty);
        if (!validation.valid) {
          toast.error('Stock insuficiente', {
            description: `${line.productDescription}: ${validation.message}`,
          });
          return false;
        }
      }
  ...
  const handleSubmit = () => {
    ...
    router.push('/inventario/ajustes?status=pendiente');
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">Nuevo Ajuste de Inventario</h1>
            <p className="text-sm text-gray-500">Crear ajuste positivo o negativo</p>
          </div>
        </div>
      </div>
          ...
          <div className="mt-6">
            ...
            <div className="flex h-24 items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 transition-colors hover:border-brand-400 hover:bg-brand-50">
              <div className="flex flex-col items-center text-gray-500">
                ...
                <span className="text-xs">Click o arrastra para subir fotos</span>
              </div>
            </div>
          </div>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-200 bg-gray-50">
                    ...
                    <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Stock Actual</th>
                    <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">
                      Cantidad {adjustmentType === 'positivo' ? 'a Sumar' : 'a Restar'}
                    </th>
                    <th className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500">Stock Resultante</th>
                    {canViewCosts && (
                ...
                <tbody className="divide-y divide-gray-100">
                  {lines.map((line, index) => {
                    const resultingStock = adjustmentType === 'positivo'
                      ...
                      <tr key={line.productId} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-right">
                          <span className={cn(
                            ...
                            isNegativeResult ? 'text-red-600' : 'text-gray-900'
                          )}>
                            {resultingStock}
                            {isNegativeResult && (
                        ...
                        {canViewCosts && (
                          <td className="px-4 py-3 text-right">
                            <span className="font-mono text-sm text-gray-700">
                              {formatCurrency(line.adjustmentQty * line.costCIF)}
                            </span>
                          </td>
                        )}
          ...
          {lines.length > 0 && (
            <div className="mt-6 flex items-center justify-between rounded-lg bg-gray-50 p-4">
              ...
              <div className="flex gap-3">
                ...
                <Button color="primary" onPress={handleSubmit} className="bg-brand-600">
                ...
                >
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500">
                      {product.reference} • Stock: {product.stock.existence}
                    </p>
                  </div>
