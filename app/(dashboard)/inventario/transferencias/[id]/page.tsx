...
export default function TransferenciaDetailPage() {
  ...
  if (!transfer) {
    return (
      <div className="flex h-96 flex-col items-center justify-center">
        ...
        <p className="text-lg font-medium text-gray-900">Transferencia no encontrada</p>
        <p className="text-sm text-gray-500">La transferencia {params.id} no existe</p>
        <Button
          variant="light"
          onPress={() => router.back()}
          className="mt-4"
  ...
  const isCreator = user?.id === transfer.createdBy;
  const canConfirmTransfer = canConfirm && !isCreator && transfer.status === 'enviada';
  const isB2BtoB2C = transfer.sourceWarehouseType === 'B2B' && transfer.destWarehouseType === 'B2C';
  ...
  const handleOpenConfirmModal = () => {
    ...
    const initialQuantities = transfer.lines.map((line) => ({
      lineId: line.id,
      receivedQty: line.resultingUnits,
      notes: '',
    }));
  ...
  const handleUpdateReceivedQty = (lineId: string, qty: number) => {
    setReceivedQuantities((prev) =>
      prev.map((rq) => (rq.lineId === lineId ? { ...rq, receivedQty: qty } : rq))
    );
  };
  ...
  const handleConfirmTransfer = () => {
    ...
    if (hasDiscrepancies) {
      toast.warning('Transferencia confirmada con discrepancias', {
        description: `Se han registrado diferencias en la recepción de ${transfer.id}.`,
      });
    } else {
      toast.success('Transferencia confirmada', {
        description: `La transferencia ${transfer.id} ha sido recibida correctamente.`,
      });
    }
  ...
  const timelineItems = [
    ...
    },
    {
      ...
      user: transfer.receivedByName,
      icon: <CheckCircle2 className="h-4 w-4" />,
      completed: transfer.status === 'recibida' || transfer.status === 'recibida_discrepancia',
    },
  ];
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-3">
                <h1 className="text-2xl font-semibold text-gray-900">{transfer.id}</h1>
                <span
                  className={cn(
              ...
              <p className="text-sm text-gray-500">Transferencia de mercancía</p>
            </div>
          </div>
        </div>
        {canConfirmTransfer && (
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            <h2 className="mb-4 text-lg font-semibold text-gray-900">Bodegas</h2>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-700">
                      Factor de inflación: {((transfer.inflationFactor - 1) * 100).toFixed(0)}%
                    </p>
                  </div>
                </div>
              </div>
              ...
              <table className="w-full">
                  ...
                  {transfer.lines.map((line) => (
                    <tr key={line.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-right">
                        <span className="text-sm text-gray-600">{line.quantityCases}</span>
                      </td>
                      <td className="px-4 py-3 text-right">
                      ...
                      {transfer.status !== 'enviada' && (
                        <td className="px-4 py-3 text-right">
                          <span
                            className={cn(
                              'text-sm font-medium',
                              line.hasDiscrepancy ? 'text-amber-600' : 'text-green-600'
                            )}
                          >
                            {line.receivedQty}
                            {line.hasDiscrepancy && (
                              <span className="ml-1 text-xs">
                                ({line.receivedQty! - line.resultingUnits})
                              </span>
                            )}
                          </span>
                        </td>
                ...
                <tfoot className="border-t border-gray-200 bg-gray-50">
                  <tr>
                    ...
                    {transfer.status !== 'enviada' && <td className="px-4 py-3"></td>}
                    {canViewCosts && (
          ...
          {transfer.hasDiscrepancies && transfer.discrepancySummary && (
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-amber-900">Discrepancias Detectadas</p>
                  <p className="mt-1 text-sm text-amber-700">{transfer.discrepancySummary}</p>
                  {transfer.lines
                    .filter((l) => l.hasDiscrepancy && l.discrepancyNotes)
                    .map((line) => (
                      <div key={line.id} className="mt-2 rounded-lg bg-white/50 p-2">
                        <p className="text-xs font-medium text-amber-800">
                          {line.productReference}:
                        </p>
                        <p className="text-xs text-amber-700">{line.discrepancyNotes}</p>
                      </div>
                    ))}
                </div>
              </div>
          ...
          {transfer.observation && (
            <div className="rounded-xl border border-gray-200 bg-white p-6">
              <h2 className="mb-2 text-sm font-medium text-gray-700">Observación</h2>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-900">{transfer.createdByName}</p>
                </div>
              </div>
              ...
              {transfer.receivedByName && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900">{transfer.receivedByName}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <ModalContent>
          <ModalHeader className="border-b border-gray-200">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Confirmar Recepción</h3>
                <p className="text-sm text-gray-500">Verifica las cantidades recibidas</p>
              </div>
            </div>
          </ModalHeader>
          <ModalBody className="py-6">
            <div className="space-y-4">
              <p className="text-sm text-gray-600">
                Ingresa las cantidades efectivamente recibidas. Si hay diferencias, se registrarán
                como discrepancias.
              </p>
              ...
              <div className="overflow-hidden rounded-lg border border-gray-200">
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {transfer.lines.map((line) => {
                      const receivedQty =
                        receivedQuantities.find((rq) => rq.lineId === line.id)?.receivedQty ||
                        line.resultingUnits;
                      const diff = receivedQty - line.resultingUnits;
                      ...
                      return (
                        <tr key={line.id}>
                          ...
                          <td className="px-4 py-3 text-right">
                            <span className="text-sm text-gray-600">{line.resultingUnits}</span>
                          </td>
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-end gap-2">
                              <Input
                                ...
                                onChange={(e) =>
                                  handleUpdateReceivedQty(line.id, parseInt(e.target.value) || 0)
                                }
                                variant="bordered"
                                size="sm"
                                classNames={{
                          ...
                          <td className="px-4 py-3">
                            <Input
                              placeholder="Notas..."
                              value={
                                receivedQuantities.find((rq) => rq.lineId === line.id)?.notes || ''
                              }
                ...
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-amber-700">
                      Se detectaron diferencias en las cantidades. La transferencia se marcará como
          ...
          <ModalFooter className="border-t border-gray-200">
            <Button variant="light" onPress={() => setIsConfirmModalOpen(false)}>
              Cancelar
            </Button>
            <Button
              color="primary"
