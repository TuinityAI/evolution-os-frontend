...
import {
  ...
  getBillsOfLadingData,
  subscribeBillsOfLading,
  getCertificatesData,
  subscribeCertificates,
  getTimelineEventsData,
  subscribeTimelineEvents,
  getExpedientById,
  getDMCsByExpedient,
  getBLByExpedient,
  getCertificatesByExpedient,
...
export default function ExpedientDetailPage() {
  ...
  // Subscribe to all relevant stores for reactivity
  ...
  const allBls = useStore(subscribeBillsOfLading, useCallback(() => getBillsOfLadingData(), []));
  const allCerts = useStore(subscribeCertificates, useCallback(() => getCertificatesData(), []));
  const allTimeline = useStore(subscribeTimelineEvents, useCallback(() => getTimelineEventsData(), []));

  const expedient = useMemo(() => getExpedientById(id), [id, allExpedients]);
  const dmcs = useMemo(() => (expedient ? getDMCsByExpedient(expedient.id) : []), [expedient, allDmcs]);
  const bl = useMemo(() => (expedient ? getBLByExpedient(expedient.id) : undefined), [expedient, allBls]);
  const certificates = useMemo(
    () => (expedient ? getCertificatesByExpedient(expedient.id) : []),
    [expedient, allCerts]
  );
  const timeline = useMemo(
    () => (expedient ? getExpedientTimeline(expedient.id) : []),
    [expedient, allTimeline]
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                >
                  {SHIPMENT_TYPE_LABELS[expedient.type]}
                </span>
                <span
                  className={cn(
                ...
                <span
                  className={cn(
                    ...
                    PRIORITY_COLOR[expedient.priority]
                  )}
                >
                  {PRIORITY_LABELS[expedient.priority]}
                </span>
              </div>
        ...
        >
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              ...
              <div className="rounded-xl border border-gray-200 dark:border-[#2a2a2a] bg-white dark:bg-[#141414] p-6">
                ...
                <dl className="space-y-3">
                  {[
                    { label: 'Tipo', value: SHIPMENT_TYPE_LABELS[expedient.type] },
          ...
          {activeTab === 'documentos' && (
            <div className="space-y-4">
              ...
              <div className="space-y-3">
                {dmcs.map((d) => {
                  const config = DMC_STATUS_CONFIG[d.status];
                  return (
                    <div
                      key={d.id}
                ...
                {bl && (
                  <div className="flex items-center justify-between rounded-xl border border-gray-200 dark:border-[#2a2a2a] bg-white dark:bg-[#141414] p-4">
                    ...
                    <span
                      className={cn(
                        'inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium',
                        BL_STATUS_CONFIG[bl.status].bg,
                        BL_STATUS_CONFIG[bl.status].text
                      )}
                    >
                      <span
                        className={cn(
                          'h-1.5 w-1.5 rounded-full',
                          BL_STATUS_CONFIG[bl.status].dot
                        )}
                      />
                      {BL_STATUS_CONFIG[bl.status].label}
                    </span>
                  </div>
                )}
                ...
                {certificates.map((cert) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900 dark:text-white">
                          {CERTIFICATE_TYPE_LABELS[cert.type]} {cert.id}
                        </p>
                        <p className="text-xs text-gray-500 dark:text-[#888888]">
                ...
                <>
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {[
                      {
                        label: 'Modo',
                        value: TRANSPORT_MODE_LABELS[expedient.transport.mode],
                        icon: Truck,
                      },
                      {
            ...
            <div className="rounded-xl border border-gray-200 dark:border-[#2a2a2a] bg-white dark:bg-[#141414] p-6">
              {timeline.length > 0 ? (
                <div className="relative">
                  <div className="absolute left-[17px] top-0 bottom-0 w-px bg-gray-200 dark:bg-[#2a2a2a]" />
                  <div className="space-y-6">
                    {timeline.map((event) => {
                      const IconComponent =
                        TIMELINE_ICONS[event.action] || Clock;
                      return (
