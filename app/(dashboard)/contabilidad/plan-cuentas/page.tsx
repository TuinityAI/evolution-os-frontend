...
function AccountRow({
  ...
  const indent = account.level === 1 ? 0 : account.level === 2 ? 24 : 48;
...
export default function PlanCuentasPage() {
  ...
  const [newType, setNewType] = useState<string>('activo');
  ...
  const expandAll = () => {
    setExpanded(new Set(MOCK_ACCOUNTS.map((a) => a.id)));
  };
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="text-center text-sm text-gray-500 dark:text-[#888888]">
        {MOCK_ACCOUNTS.length} cuentas en el plan contable
      </div>
