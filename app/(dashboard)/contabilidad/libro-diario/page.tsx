...
export default function LibroDiarioPage() {
  ...
  const entries = useMemo(() => {
    return getJournalEntries({
      ...
      source: (sourceFilter as JournalEntrySource | 'all') || undefined,
      status: statusFilter || undefined,
      dateFrom: dateFrom || undefined,
      dateTo: dateTo || undefined,
    });
  }, [searchQuery, sourceFilter, statusFilter, dateFrom, dateTo]);
  ...
  const handleSaveEntry = () => {
    ...
    if (!isBalanced) {
      toast.error('El asiento no está balanceado', {
        description: `Débito: ${formatCurrencyAccounting(totalDebit)} | Crédito: ${formatCurrencyAccounting(totalCredit)}`,
      });
      return;
    }
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-[#2a2a2a]">
              {entries.map((entry, index) => {
                const sourceColor = JOURNAL_SOURCE_COLORS[entry.source];
